...
export class VinSearchOrchestratorService {
  ...
  async search(vin: string, skipCache = false): Promise<VinSearchOrchestratorResult> {
    ...
    try {
      // Get URLs from aggregators and competitors
      const [aggregatorCandidates, competitorCandidates] = await Promise.all([
      ...
      const allCandidates: VinSearchCandidate[] = extractedData.map(data => ({
